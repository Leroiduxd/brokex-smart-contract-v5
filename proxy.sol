...
contract Trades {
    ...
    /// @notice Exécute un LIMIT -> OPEN avec prix issu de la PREUVE.
    ///         Règle d’acceptation :
    ///         - LONG  : px <= target * (1 + TOL_BPS/10000)
    ///         - SHORT : px >= target * (1 - TOL_BPS/10000)
    function executeLimit(uint32 id, bytes calldata proof) external {
        ...
        // borne de tolérance autour du target
        ...
        if (longSide) {
            require(uint256(uint64(px)) <= upper, "PRICE_NOT_BETTER_LONG");
        } else {
            require(uint256(uint64(px)) >= lower, "PRICE_NOT_BETTER_SHORT");
        }
    ...
    ///         Règles:
    ///           - TP : "prix indiqué ou mieux"
    ///               * LONG  (vente) : px >= target*(1 - tol)
    ///               * SHORT (achat) : px <= target*(1 + tol)
    ///           - SL / LIQ : "prix indiqué ou pire"
    ///               * LONG  : px <= trigger*(1 + tol)
    ///               * SHORT : px >= trigger*(1 - tol)
    function close(uint32 id, uint8 reason, bytes calldata proof) external {
        ...
        // Comparaisons en uint pour éviter les signes
        uint256 trig = uint256(uint64(triggerX6));
        uint256 pxU = uint256(uint64(px));
        uint256 upper = (trig * (10000 + TOL_BPS)) / 10000; // trigger*(1+tol)
        uint256 lower = (trig * (10000 - TOL_BPS)) / 10000; // trigger*(1-tol)

        bool longSide = (t.flags & 0x01) != 0;
        bool ok;

        if (reason == REASON_TP) {
            // "prix indiqué ou mieux"
            ok = longSide
                ? (pxU >= lower) // vendre long à ≥ target*(1 - tol) (et plus haut = mieux)
                : (pxU <= upper); // racheter short à ≤ target*(1 + tol) (et plus bas = mieux)
        } else {
            // SL / LIQ : "prix indiqué ou pire"
            ok = longSide
                ? (pxU <= upper) // long : ≤ trigger*(1 + tol) (et plus bas = pire)
                : (pxU >= lower); // short: ≥ trigger*(1 - tol) (et plus haut = pire)
        }
    ...
    ///         Règle d’acceptation identique à executeLimit().
    function execLimits(
        uint32 assetId,
        uint32[] calldata ids,
        bytes calldata proof
        ...
        uint256 pxU = uint256(uint64(px)); // pour comparaisons
        ...
        for (uint256 i = 0; i < ids.length; ) {
            ...
            if (
                ...
                bool longSide = (t.flags & 0x01) != 0;
                bool ok = longSide ? (pxU <= upper) : (pxU >= lower);
    ...
    // ───────────────── helpers internes (ajouter dans le contrat) ─────────────────
    ...
    /// Finalise la fermeture (déverrouille, settle, état + event)
    ...
    // ───────────────── closeBatch refactorisée (remplacer la fonction existante) ─────────────────
    ...
    function closeBatch(
        ...
        for (uint256 i = 0; i < ids.length; ) {
            ...
            bool ok = (reason == REASON_TP)
                ? _okTP(longSide, triggerX6, px)
                : _okSLorLIQ(longSide, triggerX6, px);
            ...
            _finalizeClose(t, ids[i], px, longSide, rm);
            unchecked {
                ++closed;
                ++i;
            }
